...
grep '[[:alpha:]]' $fichier_cle | sort -k3,3 > tresor.txt

head -n 2 tresor.txt > tresor_inter.txt
tail -n 2 tresor.txt >> tresor_inter.txt

mots=$(cut -d' ' -f3 tresor_inter.txt)

rm tresor.txt tresor_inter.txt
...
for chemin in $mots
do
    tresor=$tresor/$chemin
done

echo Contenu du trésor :
echo
cat $tresor
